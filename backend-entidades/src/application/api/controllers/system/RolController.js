...
module.exports = function setupRolController (services) {
  ...
  async function findOne (req, res) {
    try {
      ...
      const respuesta = await RolService.findOne(params);
  ...
  async function createItem (req, res) {
    try {
      ...
      const respuesta = await RolService.createOrUpdate(datos);
      return res.status(200).send(new Respuesta('OK', Finalizado.OK, respuesta));
    } catch (error) {
      return res.status(error.httpCode || HttpCodes.userError).json(new Respuesta(error.message, Finalizado.FAIL));
    }
  ...
  async function deleteItem (req, res) {
    try {
      const respuesta = await RolService.deleteItem(req.params.id);
